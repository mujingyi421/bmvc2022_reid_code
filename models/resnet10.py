...
class ResNet50_10(nn.Module):
    def __init__(self, res4_stride=1, **kwargs):
        ...
        self.conv_1 = nn.Conv2d(512, 3, kernel_size=1, padding=0, bias=False)
        self.conv_2 = nn.Conv2d(1, 1, kernel_size=3, padding=1, bias=False)
        self.conv_3 = nn.Conv2d(4, 1, kernel_size=7, padding=3, bias=False)
    ...
    def forward(self, xx):
        ...
        y = self.conv_3(mask) #[b,1,32,16]

        y = torch.sigmoid(y) 
        # draw_features(y.cpu().detach().numpy())
